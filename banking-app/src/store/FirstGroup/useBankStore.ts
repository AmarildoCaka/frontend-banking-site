import {
  amountStateFunction,
  withdrawStateFunction,
  depositHistoryUnitFunct,
  withdrawHistoryUnitFunct,
  loadBalancesFromLocalStorage,
  timeStatementFunction,
  resetToZeroCapturedTimeDataFunct,
  dayMonthYearDataFunct,
  capturedTimeFunct,
  isActiveStateFunct,
  graphDataFunct,
  depostitMsgFunct,
  firstAmountVisibilityStorageFunct,
  secondAmountVisibilityStorageFunct,
  thirdAmountVisibilityStorageFunct,
  generalAccountStateStoreFunct,
  generalCardListFunct,
  dashboardModeFunct,
} from "./stateFuncts";
...
export const useBankStore = create<BankState>((set, get) => {
  return {
    ...
    services: [
      {
        ...
        serviceText:
          "Access flexible, low-interest loans for personal needs, with easy approval and repayment terms.",
...

      {
        ...
        serviceText:
          "Grow your wealth with tailored investment strategies designed to maximize returns and minimize risks.",

        feature: {
          firstFeature:
            "Tailored investment strategies to match your risk tolerance.",
...

      {
        ...
        serviceText:
          "Manage your finances seamlessly with our wide range of credit and debit card options, offering rewards and benefits.",
...

      {
        ...
        serviceText:
          "Comprehensive wealth management services to help you achieve long-term financial stability and growth.",

        feature: {
          firstFeature: "Personalized financial planning and advice.",

          secondFeature:
            "Retirement and estate planning to ensure your future is secure.",
...

      {
        ...
        serviceText:
          "Protect yourself and your loved ones with our customizable insurance solutions, covering health, life, and property.",

        feature: {
          firstFeature: "Tailored insurance plans to fit your specific needs.",

          secondFeature:
            "Competitive premiums with extensive coverage options.",
...

      {
        ...
        serviceText:
          "Affordable home and auto loans with competitive rates and flexible terms to help you secure your dream home or vehicle.",
    ...
    deleteSingleDepositUnit: (id) => {
      set((state) => {
        const updated = state.depositHistoryUnitState.filter(
          (d) => d.id !== id,
        );
    ...
    deleteSingleWithdrawUnit: (id) => {
      set((state) => {
        const updated = state.withdrawHistoryUnitState.filter(
          (d) => d.id !== id,
        );
    ...
    resetAmountToZeroTimeCapture: () => {
      ...
      localStorage.setItem(
        "dayMonthYearEraseData",
        JSON.stringify(day_month_year_object),
      );
    },
    ...
    depositConfirmationMsgFunct: () => {
      ...
      localStorage.setItem(
        "storedMessageData",
        JSON.stringify(reversedMsgData),
      );
    },
    ...
    setFirstAmountVisibilityFunct: () => {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(innerDataUtilisation),
      );
    },
    ...
    initializeGeneralAccountState: () => {
      const savedState = JSON.parse(
        localStorage.getItem("generalAccountStateStorage") || '"Enabled"',
      );
    ...
    generalAccountBlockingFunct: () => {
      const stateArray = [
        get().firstBalanceActivityState,
        get().secondBalanceActivityState,
        get().thirdBalanceActivityState,
        get().generalAccountState,
      ];
      ...
      const [
        ...
        get().firstDepositStateFunct,
        get().secondDepositStateFunct,
        get().thirdDepositStateFunct,
        get().accountFreezingFunct,
        get().accountUnfreezingFunct,
      ];
