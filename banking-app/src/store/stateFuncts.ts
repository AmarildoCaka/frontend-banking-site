import { depositHistoryInterface, withdrawHistoryInterface, dayMonthYearStringInterface, graphDataInterface, Card, loanUnitInterface, generalLoanDetailsInterface, opinionMsgListInterface, starRatingListInterface, rateMsgSubmitTimeListInterface, dashboardDataStateType } from './functInterfaces';
...
export const transactionFunct = (): Transaction[] => {
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load transactions data from localStorage: ${error}`);
...
export const loadBalancesFromLocalStorage = (): Record<'USD' | 'EUR' | 'GBP', number> => {
...
export const resetToZeroCapturedTimeDataFunct = (): string | null => {
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load the reset to zero initiation time from localStorage: ${error}`);
...
export const graphDataFunct = (): graphDataInterface[] => {
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load captured graph data from localStorage: ${error}`);
...
export const depostitMsgFunct = (): boolean => {
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load captured message data from localStorage: ${error}`);
...
export const firstAmountVisibilityStorageFunct = (): boolean => {
  ...
  try
  {
    ...
    const storedAmountVisibility = localStorage.getItem('firstStoredAmountVisibility');
  ...
  catch(error) 
  {
    ...
    console.error(`Failed to load amount visibility data from localStorage: ${error}`);
...
export const secondAmountVisibilityStorageFunct = (): boolean => {
  ...
  try
  {
    ...
    const storedAmountVisibility = localStorage.getItem('secondStoredAmountVisibility');
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load second amount visibility data from localStorage: ${error}`);
...
export const thirdAmountVisibilityStorageFunct = (): boolean => {
  ...
  try
  {
    ...
    const storedAmountVisibility = localStorage.getItem('thirdStoredAmountVisibility');
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load third amount visibility data from localStorage: ${error}`);
...
export const generalAccountStateStoreFunct = (): 'Enabled' | 'Disabled' => {
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load general account state data from localStorage: ${error}`);
...
export const generalCardHistoryFunct = (): Card[] => {
  ...
  try
  {
    ...
    const cardHistoryListStorage = localStorage.getItem('cardHistoryLinkingSystem');
  ...
  catch(error)
  {
    ...
    console.error(`Failed to load card history data from localStorage: ${error}`);
...
export const generalLoanDetailsFunct = (): generalLoanDetailsInterface[] => {
  ...
  try
  {
    ...
    const generalLoanDetailsStorage = localStorage.getItem('generalLoanDetailsStorage');
