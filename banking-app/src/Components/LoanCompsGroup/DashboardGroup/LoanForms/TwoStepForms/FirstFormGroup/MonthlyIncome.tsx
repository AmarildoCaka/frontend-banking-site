...
const MonthlyIncomeComp = () => {
  ...
  const { monthlyIncome, setMonthlyIncome, firstStepFormFieldErrors } =
    useConditionalBankStore();

  const { theme } = useThirdBankStore();

  const { copyMonthlyIncomeFormFunct, emptyMonthlyIncomeFormFunct } =
    ...
    if (firstStepFormFieldErrors.monthlyIncome === true) {
      return (
        <p className="text-red-500 font-semibold text-md">
          Empty monthly payment field, please fill this form
        </p>
      );
  ...
  return (
    <>
      ...
      <section className="flex flex-col justify-evenly gap-2">
        ...
        <div
          className={`loan-monthly-income-form flex items-center w-full rounded-md overflow-hidden transform transition duration-300 hover:scale-102 border ${mainConditionLogic}`}
        >
          <input
            type="number"
            value={monthlyIncome}
          ...
          <button
            type="button"
            className="loan-monthly-income-copy-btn cursor-pointer tarnsaform transition duration-300 hover:scale-110 px-3 py-2"
            onClick={() => {
          ...
          <button
            type="button"
            className="loan-monthly-income-erase-btn cursor-pointer tarnsaform transition duration-300 hover:scale-110 px-3 py-2"
            onClick={() => {
