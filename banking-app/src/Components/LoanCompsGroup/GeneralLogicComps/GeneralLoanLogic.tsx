...
import { loanUnitInterface, generalLoanDetailsInterface } from '../../../store/functInterfaces';
  ...
  const copyDescriptionFunct = async () => {
    ...
    switch(true)
    {
      ...
      case loanForm === "":
        ...
        showPopUpMessage("No description, please write a description first", "error");
...
export const useSubmitEstimatedLoanHook = () => {
  ...
  const setLoanFirstName = useConditionalBankStore((state) => state.setLoanFirstName);

  const loanLastName = useConditionalBankStore((state) => state.loanLastName);

  const setLoanLastName = useConditionalBankStore((state) => state.setLoanLastName);
  ...
  const setMonthlyIncome = useConditionalBankStore((state) => state.setMonthlyIncome);

  const monthlyIncomeCurrency = useConditionalBankStore((state) => state.monthlyIncomeCurrency);

  const setMonthlyIncomeCurrency = useConditionalBankStore((state) => state.setMonthlyIncomeCurrency);

  const loanCurrency = useConditionalBankStore((state) => state.loanCurrency);

  const setLoanCurrency = useConditionalBankStore((state) => state.setLoanCurrency);

  const loanForm = useConditionalBankStore((state) => state.loanForm);

  const setLoanRestrictionNumber = useConditionalBankStore((state) => state.setLoanRestrictionNumber);
  ...
  const setLoanFormError = useConditionalBankStore((state) => state.setLoanFormError);

  const setTrueSecondStepFormFieldErrors = useConditionalBankStore((state) => state.setTrueSecondStepFormFieldErrors);
  ...
  const setLoanSecondStepToTrue = useConditionalBankStore((state) => state.setLoanSecondStepToTrue);

  const setFirstLoanForm = useConditionalBankStore((state) => state.setFirstLoanForm);

  const setSecondLoanForm = useConditionalBankStore((state) => state.setSecondLoanForm);

  const setGeneralLoanProgressBarToFalse = useConditionalBankStore((state) => state.setGeneralLoanProgressBarToFalse);

  const generalLoanDetails = useConditionalBankStore((state) => state.generalLoanDetails);

  const setGeneralLoanDetailsFunct = useConditionalBankStore((state) => state.setGeneralLoanDetailsFunct);

  const monthlyPayment = Number((loanAmount / Number(loanTerm) + loanAmount * 0.05).toFixed(2));

  const secondMonthlyPayment = (loanAmount / Number(loanTerm) + loanAmount * 0.05).toFixed(2);

  const interest = ((monthlyPayment * Number(loanTerm)) - loanAmount).toLocaleString();
  ...
  const submitEstimatedLoan = () => {
    ...
    if(loanForm.trim() === "" || loanTerm === 0 || loanAmount === 0 || loanCurrency === '')
    {
  
      setTrueSecondStepFormFieldErrors();

      showPopUpMessage("Cannot set a loan, please finish all the above steps first", "error");
