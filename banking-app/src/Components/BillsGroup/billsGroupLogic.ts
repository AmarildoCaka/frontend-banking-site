...
export const useBillsLogicCustomHook = () => {
  ...
  const setSelectedBills = useFourthBankStore(
    (state) => state.setSelectedBills,
  );

  const billCategoriesObj = useFourthBankStore(
    (state) => state.billCategoriesObj,
  );

  const generalBillsElement = useFourthBankStore(
    (state) => state.generalBillsElement,
  );

  const setGeneralBillsElement = useFourthBankStore(
    (state) => state.setGeneralBillsElement,
  );

  const setPaidBills = useFourthBankStore((state) => state.setPaidBills);

  const setDeleteAllBills = useFourthBankStore(
    (state) => state.setDeleteAllBills,
  );

  const setDeleteOneBill = useFourthBankStore(
    (state) => state.setDeleteOneBill,
  );
  ...
  const getStatusBadge = (status: string) => {
    if (status === "paid") {
      return {
        ...
        className:
          "flex flex-row justify-evenly items-center text-center gap-x-2 bg-green-100 text-green-700 border border-green-700 px-2 py-1 rounded-full text-xs font-semibold p-1",
      ...
      return {
        ...
        className:
          "flex flex-row justify-evenly items-center text-center gap-x-2 bg-yellow-100 text-yellow-700 border border-yellow-700 px-2 py-1 rounded-full text-xs font-semibold p-1",
      ...
      return {
        ...
        className:
          "flex flex-row justify-evenly items-center text-center gap-x-2 bg-red-100 text-red-700 border border-red-700 px-2 py-1 rounded-full text-xs font-semibold p-1",
      ...
      return {
        ...
        className:
          "flex flex-row justify-evenly items-center text-center gap-x-2 bg-gray-100 text-gray-700 border border-gray-700 px-2 py-1 rounded-full text-xs font-semibold p-1",
  ...
  const handlePaySelected = (listData: billInterface[]) => {
    ...
    showPopUpMessage(
      `Successfully paid ${billsToPay.length} ${numOfPayedBillsCountData} totaling $${totalAmount.toFixed(2)}`,
      "success",
    );
  };
