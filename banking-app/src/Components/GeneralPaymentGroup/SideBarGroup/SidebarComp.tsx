...
import FifthListComp from "./SideBarBtnGroup/FifthListComp";
...
const SidebarComp = () => {
  ...
  const { activeRoute } = useConditionalBankStore();
  ...
  return (
    <>
      ...
      <button
        type="button"
        className={`md:hidden fixed top-24 left-0 z-50 w-8 h-8 flex items-center justify-center bg-indigo-600 text-white rounded-r shadow-lg transform transition-transform duration-300 ${isOpen ? "translate-x-64" : "translate-x-0"}`}
        onClick={() => {
          setIsOpen((prev) => !prev);
        }}
      >
      ...
      <aside
        className={`main-dashboard-wrapper fixed md:static top-0 left-0 h-screen w-64 bg-gray-800 text-white z-40 flex flex-col transform transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-64 md:translate-x-0"} ${activeRoute !== "" ? "mt-30" : "mt-20"} overflow-hidden`}
      >
        ...
        <nav className="space-y-5 overflow-y-auto overflow-x-hidden custom-scrollbar px-5 flex-1">
          <div>
            <h3 className="font-bold text-lg mb-2">Balances & Transactions</h3>
