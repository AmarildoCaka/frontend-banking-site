...
const AlertPopUpComp = ({ indexData }: { indexData: number }) => {
  ...
  const { setMsgAlertPopUpUnit, opinionMsgList, editText, setOpinionMsgList, setEditingId, setMsgEditUnit, editingId } = useThirdBankStore();
  ...
  const handleSave = (id: number) => {
    ...
    const updated = opinionMsgList.map((msg) => (msg.id === id)? { ...msg, message: editText }: msg);
  ...
  const msgEditLastStepFunct = () => {
    ...
    try
    {
      ...
      showPopUpMessage(`Message number ${indexData} edited successfully`, 'success');
    ...
    catch(error)
    {
      ...
      alert(`Upss, something went wrong, cannot edit message number ${indexData}`);
    ...
    <>
      ...
      <section className="fixed inset-0 bg-opacity-50 flex items-center justify-center backdrop-blur-sm z-50">
        ...
        <div className="rating-msg-alert-wrapper w-[500px] h-72 flex flex-col justify-between place-items-center gap-1 shadow-2xl rounded-xl text-center p-4">
          ...
          <h1 className="text-red-500 font-bold text-2xl mb-4">Do you want to edit this message?</h1>
          
          <p className="rating-msg-alert-description font-semibold mb-2">Are you sure you want to edit message number {indexData + 1}? Press Yes to edit this message, or No to go back.</p>
