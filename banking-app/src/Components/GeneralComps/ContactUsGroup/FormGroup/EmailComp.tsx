...
const EmailComp = () => {
  ...
  const { copyEmailFunct, emptyEmailFunct, emailFormValidationFunct } =
    useGeneralLogicHook();
      ...
      <section className="mb-3">
        <div className="relative">
          <input
            ...
            value={email}
            maxLength={20}
            className={`rating-email-form w-full border ${errors.email ? "border-red-500" : "border-gray-300"} rounded-md shadow-md transform transition duration-300 hover:scale-105 placeholder:font-semibold dark:bg-white dark:text-black p-3 ${theme === "light" || theme === "system" ? "placeholder:text-black" : theme === "dark" ? "placeholder:text-white" : "placeholder:text-dark"}`}
            name="email"
          ...
          <div className="flex flex-row justify-center place-items-center absolute top-1/2 right-2 -translate-y-1/2">
            <button
              type="button"
              className="email-copy-btn font-bold text-center rounded-lg cursor-pointer transition transform duration-300 hover:scale-105 px-2"
              onClick={() => {
                copyEmailFunct();
              }}
            >
            ...
            <button
              type="button"
              className="email-erase-btn font-bold text-center rounded-lg cursor-pointer transition transform duration-300 hover:scale-105 px-2"
              onClick={() => {
                emptyEmailFunct();
              }}
            >
        ...
        <p
          className={`${email.length === 0 ? "text-red-500" : email.length === 20 ? "text-red-500" : email.length > 0 && email.length < 20 ? "text-green-500" : "text-red-500"} text-sm font-bold mt-1`}
        >
          {emailData}
        </p>
      </section>
