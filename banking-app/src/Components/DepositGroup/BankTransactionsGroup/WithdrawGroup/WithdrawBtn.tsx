...
const WithdrawBtnComp = () => {
  const [withdrawHistoryUnit, setWithdrawHistoryUnitChange] = useState<
  ...
  const {
    ...
    loadSavedTime,
    capturedTimeData,
    setWithdrawHistoryUnit,
  } = useBankStore();

  const { withdrawAmount, withdrawCurrency, setWithdrawErrorMessageState } =
  ...
  const withdrawInUSD = () => {
    if (balances.USD >= withdrawAmount) {
      ...
      showPopUpMessage(
        `Withdraw Successful. Withdrew ${withdrawAmount} ${withdrawCurrency}.`,
        "success",
      );
    } else {
    ...
    switch (true) {
      case withdrawCurrency === "USD":
        if (withdrawAmount > 0) {
          ...
          setWithdrawErrorMessageState(true);

          showPopUpMessage(
            "Cannot make a withdraw with no amount, set amount first",
            "error",
          );
        } else {
          return null;
  ...
  return (
    <>
      <div className="text-center p-1">
        <button
          type="button"
          className={`${withdrawAmount > 0 ? "text-white bg-indigo-600 cursor-pointer" : withdrawAmount === 0 ? "text-gray-400 bg-gray-300 cursor-not-allowed" : "text-gray-400 bg-gray-300 cursor-not-allowed"} font-bold w-full rounded-md transform transition duration-300 hover:scale-105 py-3`}
          onClick={() => {
