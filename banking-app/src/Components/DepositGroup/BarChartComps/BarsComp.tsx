...
const BarsComp = () => {
  ...
  const chartData = useMemo(() => {
    const monthlyData = months.map((month, index) => {
      const depositsInMonth = depositHistoryUnitState.filter((deposit) => {
        ...
        if (
          isNaN(date.getTime()) &&
          /^\d{2}:\d{2}:\d{2}$/.test(deposit.timeDisplayState)
        ) {
      ...
      const withdrawsInMonth = withdrawHistoryUnitState.filter((withdraw) => {
        ...
        if (
          isNaN(date.getTime()) &&
          /^\d{2}:\d{2}:\d{2}$/.test(withdraw.timeDisplayState)
        ) {
          const today = new Date();
      ...
      return {
        ...
        depositAmount: depositsInMonth.reduce(
          (sum, item) => sum + (item.amount || 0),
          0,
        ),

        withdrawAmount: withdrawsInMonth.reduce(
          (sum, item) => sum + (item.withdrawAmount || 0),
          0,
        ),
      };
    });
  ...
  const filteredChartData =
    selectedMonth === null ? chartData : [chartData[selectedMonth]];
