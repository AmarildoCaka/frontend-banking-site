...
const TransactionsPieComp = () => {
  ...
  const transactionsCoparisonList = [
    { name: "Deposits", list: depositsElement },

    { name: "Withdraws", list: withdrawsElement },
  ];
        ...
        {depositsElement === 0 && withdrawsElement === 0 ? (
          <>
            <div className="h-[300px] w-full flex flex-col items-center justify-center bg-gray-50 dark:bg-[#2a2a2a] rounded-xl border border-dashed border-gray-300 dark:border-gray-600">
              ...
              <p className="text-sm text-gray-400 mt-1">
                Make a deposit or withdrawal to see insights
              </p>
            </div>
          </>
        ) : (
          <>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={transactionsCoparisonList}
                  dataKey="list"
                  nameKey="name"
                  innerRadius={60}
                  outerRadius={100}
                  label={({ list, index }) =>
                    `${list} ${transactionsCoparisonList[index].name}`
                  }
                >
                  {transactionsCoparisonList.map((entry, index) => (
                    <>
                      <Cell
                        key={index}
                        fill={transactionsCoparisonListColor[index]}
                      />
                    </>
                  ))}
                </Pie>
