...
const DepositBlockConfirm = () => {
  const {
    ...
    alertMessage,
    showPopUpMessage,
    depostiConfirmationMsg,
  } = useBankStore();

  const { setConfirmDepositState, waringPopUpState, selectedDepositId } =
    ...
    switch (true) {
      case depostiConfirmationMsg === true:
        mainArrData.find((i) => {
          ...
          if (data) {
            showPopUpMessage(
              `Deposit number ${i.id + 1} (${i.labelKey} deposit) enabled successfully`,
              "info",
            );
          }
        });
      ...
      case depostiConfirmationMsg === false:
        mainArrData.find((i) => {
          ...
          if (data) {
            showPopUpMessage(
              `Deposit number ${i.id + 1} (${i.labelKey} deposit) disabled successfully`,
              "info",
            );
          }
        });
        ...
        <div className="relative flex flex-col justify-evenly place-items-center text-center w-[470px] h-[200px] rounded-2xl shadow-2xl overflow-x-auto space-y-4 animate-fadeIn bg-gradient-to-r from-gray-50 to-blue-50 border border-blue-100">
          <section className="flex flex-col justify-evely place-items-center text-center gap-2 p-1">
            ...
            <p className="font-semibold text-md">
              Do you want to {waringPopUpState} this deposit?
            </p>
          </section>
          ...
          <section className="flex flex-row justify-between placa-items-center text-center gap-5 p-1">
            <button
              type="button"
              className="text-white font-bold bg-green-500 text-center rounded-lg shadow-2xl cursor-pointer tarnsform transition duration-300 hover:scale-105 hover:bg-green-600 px-4 py-1"
              onClick={() => {
            ...
            <button
              type="button"
              className="text-white font-bold bg-red-500 text-center rounded-lg shadow-2xl cursor-pointer tarnsform transition duration-300 hover:scale-105 hover:bg-red-600 px-4 py-1"
              onClick={() => {
                setConfirmDepositState();
              }}
            >
