...
const LineChartComp = () => {
  const [hoveredMonth, setHoveredMonth] =
    useState<userGrowthDataInterface | null>(null);

  const { userGrowthSwitch, userGrowthData, setUserGrowthSwitch } =
    useBankStore();

  const currentMonthIndex = new Date().getMonth();

  const currentMonthData = userGrowthData.find(
    (item) => item.index === currentMonthIndex,
  );
          ...
          <div className="w-full flex flex-col sm:flex-row justify-between gap-4 mt-4 p-4 bg-gradient-to-br from-indigo-50 to-indigo-100/50 dark:from-slate-800/70 dark:to-slate-700/40 rounded-xl border border-indigo-200/50 dark:border-indigo-500/20 shadow-md">
            <div className="flex flex-col justify-center items-center w-full sm:w-1/2 bg-white/60 dark:bg-slate-900/30 backdrop-blur-sm rounded-lg p-4 border border-indigo-100/40 dark:border-indigo-500/20 shadow-inner transition-all duration-300 hover:scale-[1.02]">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {hoveredMonth ? hoveredMonth.month : "Selected Month"}
              </span>

              <span className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-indigo-500 dark:from-indigo-400 dark:to-indigo-300">
                {hoveredMonth ? hoveredMonth.users : "—"}
              </span>
            </div>
            ...
            <div className="flex flex-col justify-center items-center w-full sm:w-1/2 bg-white/60 dark:bg-slate-900/30 backdrop-blur-sm rounded-lg p-4 border border-indigo-100/40 dark:border-indigo-500/20 shadow-inner transition-all duration-300 hover:scale-[1.02]">
              ...
              <span className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-indigo-500 dark:from-indigo-400 dark:to-indigo-300">
                {currentMonthData ? currentMonthData.users : 0}
              </span>
            </div>
          </div>
        </div>
